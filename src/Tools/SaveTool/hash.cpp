          ...
          int encryptmode)
{
    ...
    //if ((ret = build_hash(filehash, data, len, alignedLen, // EMCC_CHANGE: Copied from https://github.com/BrianBTB/SED-PC/blob/master/SED/hash.cpp#L68
           ...
           unsigned char *cryptkey)
{
    ...
    if (sceSdSetIndex_(ctx1, mode & 0xFF) < 0)
        ...
        if (sceSdRemoveValue_(ctx1, data, alignedLen) < 0)
                return -2;
        //if (sceChnnlsv_C4C494F8_(&ctx1, output, cryptkey) < 0) // EMCC_CHANGE
